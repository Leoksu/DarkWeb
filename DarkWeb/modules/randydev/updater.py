...
@ren.on_message(filters.command("update", cmd) & filters.me)
async def upstream(client: Client, message: Message):
    ...
    if HEROKU_API_KEY is not None:
        ...
        try:
            ...
            pass
        await status.edit(
            "`DarkWeb Successfully Updated! DarkWeb can be used again.`"
        )
    else:
        try:
            ups_rem.pull(ac_br)
...
@ren.on_message(filters.command("goupdate", cmd) & filters.me)
...
add_command_help(
    "updater",
    [
        ["update", "To see a list of the latest updates from DarkWeb."],
        ["update deploy", "To update DarkWeb."],
    ],
)
