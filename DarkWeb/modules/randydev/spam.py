...
async def delayspam(client: Client, message: Message):
    if message.chat.id in BLACKLIST_CHAT:
        return await edit_or_reply(
            message, "**This command is prohibited from being used in this group**"
        )
    ...
    await client.send_message(
        message.from_user.id, "**#DELAYSPAM**\nDelaySpam was executed successfully"
    )
...
add_command_help(
    "spam",
    [
        [f"spam <amount of spam> <text>", "Spamming texts in chats!!"],
        [f"delayspam <second> <amount of spam> <text>", "Send spam text with a specified delay period!"],
    ],
)
